...
@section('content')
	@if ( @$entry )
		<form id="editBlogEntryForm" method="post" action="{{ route( 'admin.blog.entry.view', [ $blog->id, $entry->id ] ) }}" data-form-ajax autocomplete="off" data-form-wait-title="Adding blog entry...">
			<div class="row">
				<div class="small-12 columns">
					...
					<div class="tabs-content" data-tabs-content="entryTabs">
						<div id="entryTab"class="tabs-panel is-active">
							<div class="secondary callout">
								...
								<div class="small-12">
									<label>Blog:<input type="text" value="{{ $entry->blog->name }}" disabled="disabled"></label>
								</div>

								<div class="small-12">
									<label>Name:<input class="first-focus" name="name" type="text" placeholder="Name" data-url-slug="input[name=slug]" data-form-initial-value="{{ $entry->name }}"></label>
								</div>
								...
								<div class="small-12">
									<label>
										...
										<div class="input-group">
											<span class="input-group-label">/{{ $entry->blog->slug }}/</span>
											<input class="input-group-field" name="slug" type="text" placeholder="URL" data-form-initial-value="{{ $entry->slug }}">
										</div>
								...
								<div class="small-12">
									<label>Content:<textarea class="fontFixedWidth" name="content" rows="20" data-form-initial-value="{{ $entry->content }}"></textarea></label>
								</div>
							</div>
							...
							<div class="small-12 hide">
								<div class="secondary callout">
									...
									<label><input name="active" type="checkbox" data-form-initial-value="{{ $entry->active }}">This post is active</label>
									<label><input name="hidden" type="checkbox" data-form-initial-value="{{ $entry->hidden }}">This post is hidden (can only be accessed if the visitor knows the URL)</label>
									<label><input name="restricted" type="checkbox" data-form-initial-value="{{ $entry->restricted }}">Users must be logged in to see this entry</label>
									<label><input name="comments" type="checkbox" data-form-initial-value="{{ $entry->allow_comments }}">Allow comments</label>
								</div>
							</div>
								...
								<div class="secondary callout" data-select-all>
									...
									<div class="expanded row">
										@foreach ( $blog->categories as $category)
											<label class="small-4 columns end"><input class="input" type="checkbox" name="categories[]" value="{{ $category->id }}" data-select-all="categories" data-form-initial-value="{{ $entry->categories->contains( 'blog_category_id', $category->id ) }}"> {{ $category->name }}</label>
										@endforeach
									</div>
								</div>
							@endif
							...
							<div class="small-12">
								...
								@if ( session()->has( 'blogViewEntryReferer' ) ) <a class="button secondary" href="{{ session()->get( 'blogViewEntryReferer' ) }}" title="Return to the entry">Return to Post</a> @endif
								<a class="button secondary" href="{{ route('admin.blog.view', $entry->blog->id ) }}" title="Cancel">Cancel</a>
							</div>
						</div>
		...
		<div id="deleteBlogEntryModal" class="reveal" data-reveal>
			<form id="deleteBlogEntryForm" method="post" action="{{ route( 'admin.blog.entry.delete', [ $blog->id, $entry->id ] ) }}" data-form-ajax data-form-wait-title="Deleting post...">
				<h2>Delete Post?</h2>
				<p><strong>Are you sure you want to delete the post "{{ $entry->name}}"?</strong> This action cannot be undone.</p>

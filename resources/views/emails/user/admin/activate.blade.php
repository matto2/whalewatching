	...
	<table width="600" cellpadding="0" cellspacing="0" style="border:1px solid #7599b3; padding:10px;">
		...
		<tr>
			<td style="">
				...
				<p>We have created an account for you under the email address <strong>{{ $user['email'] }}</strong></p>
				<p>Before you can begin using your account, we need you to verify your identity. In order to do that, <a href="{{ route('user.activate', $user['activation_code'] ) }}">please click here to activate your account</a>.</p>
				<p>If the link above does not work, please paste the following address into your browser's address bar:</p>
				<p><em>{{ route('user.activate', $user['activation_code'] ) }}</em></p>
			</td>
		</tr>
	</table>
</body>
